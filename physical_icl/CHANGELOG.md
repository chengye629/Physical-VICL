...
- set GPT-5.5 as the annotator for the current WISA test_v2 and full-shard campaign; local Qwen launchers are historical tooling only;
...
- retained an optional legacy alpha-3 Pass B-only local-model launcher for historical comparison while keeping protocols and output roots separate;
...
- added a separate enhanced local-model launcher for historical reproducibility;
