#!/usr/bin/env python3
"""Judge wisa_test100 and phyground using GPT-5.5 via Azure proxy.

Usage:
    python run_judge_5api.py --all
    python run_judge_5api.py --benchmark wisa_test100
    python run_judge_5api.py --benchmark phyground
"""
...
BENCHMARKS = ["wisa_test100", "phyground"]
    ...
    for attempt in range(5):
        try:
            ...
            if attempt < 4:
                ...
                continue
            return {"error": repr(e)[:500], "physical_adherence": None}
            ...
            for line in f:
                try:
                    ...
                    if r.get("physical_adherence") is not None:
                        done.add(f"{r.get('benchmark')}_{r.get('task_name')}")
                except:
                    pass
    return done
    ...
    with open(out_path, "a", encoding="utf-8") as f:
        for i, item in enumerate(todo):
            ...
            try:
                ...
                rec = {"benchmark": benchmark, "case_id": item["case_id"],
                       "task_name": task, "episode_name": item["episode_name"],
                       "judge_model": JUDGE_MODEL,
                       "physical_adherence": None, "error": f"download: {e}"}
            ...
            if not frames:
                rec = {"benchmark": benchmark, "case_id": item["case_id"],
                       "task_name": task, "episode_name": item["episode_name"],
                       "judge_model": JUDGE_MODEL,
                       "physical_adherence": None, "error": "no_frames"}
                f.write(json.dumps(rec, ensure_ascii=False) + "\n")
                f.flush()
                n_err += 1
...
def main():
    ...
    ap.add_argument("--cache_dir", default="/tmp/physical_icl_cache")
